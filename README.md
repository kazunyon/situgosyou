...
各項目には編集可能な番号を付けられます。タイトルと意味・説明を、文字入力または音声入力で登録でき、大切なメモには星マークを付けられます。Supabaseを設定すると、同じメールアドレスでログインしたPCとスマートフォンで、同じメモを利用できます。
...
番号機能を追加するため、アプリを公開する前に次の作業を1回だけ行います。既存のメモは削除されません。
...
4. **Run** を押します。
5. エラーが表示されなければ完了です。既存のメモには現在の表示順で`1、2、3…`の番号が設定されます。
...
| 番号 | `display_number integer` | 一覧に表示する1〜9999の番号です。 |

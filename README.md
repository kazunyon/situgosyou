...
各項目の表示番号とカテゴリは別々に管理されます。全体の`すべて／マーク`表示とは独立して、カテゴリで絞り込めます。カテゴリ名は設定画面から変更でき、`4 食べ物`などを最大5件まで追加できます。タイトルと意味・説明を、文字入力または音声入力で登録でき、大切なメモには星マークを付けられます。Supabaseを設定すると、同じメールアドレスでログインしたPCとスマートフォンで、同じメモを利用できます。
...
5. エラーが表示されなければ完了です。既存のメモはカテゴリ1に設定され、初期カテゴリとして`1 自然／2 乗り物／3 AI`が自動作成されます。

...
各項目の表示番号とカテゴリは別々に管理されます。全体の`すべて／マーク`表示とは独立して、`1 自然／2 乗り物／3 AI`のカテゴリで絞り込めます。タイトルと意味・説明を、文字入力または音声入力で登録でき、大切なメモには星マークを付けられます。Supabaseを設定すると、同じメールアドレスでログインしたPCとスマートフォンで、同じメモを利用できます。
...
5. エラーが表示されなければ完了です。既存のメモはカテゴリ`1 自然`に設定され、画面からあとで変更できます。
...
| カテゴリ | `category_number integer` | 表示番号とは独立したカテゴリです。`1=自然`、`2=乗り物`、`3=AI`です。 |

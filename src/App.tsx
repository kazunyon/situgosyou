  ...
  return <main className="app-shell">
    ...
    <section className={`memo-list ${section === 'pc-linux' ? 'guide-list' : ''}`} aria-live="polite">
      {loading ? <p className="status">読み込み中…</p> : displayed.length === 0 ? <p className="status">まだ{section === 'daily' ? 'メモ' : '操作項目'}がありません。<br />上のボタンから追加できます。</p> : displayed.map((memo, index) => section === 'daily' ? <article className="memo-row" key={memo.id}>
        ...
        <button className="memo-content" onClick={() => openEdit(memo)}><strong className={`memo-title title-color-${memo.titleColor}`}>{memo.title}</strong><span className="memo-category">{memo.categoryNumber} {categoryName(categories, memo.categoryNumber)}</span>{memo.meaning && <span className="memo-meaning">{memo.meaning}</span>}</button>

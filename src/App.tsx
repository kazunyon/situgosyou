...
function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="intro"><h2>思い出したいことを、すぐに。</h2><p>{section === 'daily' ? 'ことばでも、文章でも書けます。' : '画像を見ながら、PCの操作を確認できます。'}</p></section>
    <nav className="section-tabs" aria-label="大分類"><button className={section === 'daily' ? 'selected' : ''} onClick={() => changeSection('daily')}><MessageSquareText size={21} />日常用</button><button className={section === 'pc-linux' ? 'selected' : ''} onClick={() => changeSection('pc-linux')}><Laptop size={22} />PC/Linux用</button></nav>
    <label className="search-box"><Search size={24} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={section === 'daily' ? '日常用をさがす' : 'PC/Linuxの操作をさがす'} aria-label="メモをさがす" /></label>
    ...
    {section === 'daily' ? <><div className="filter-group"><span className="filter-group-label">表示</span><nav className="filter-tabs" aria-label="日常用の表示切り替え"><button className={filter === 'all' ? 'selected' : ''} onClick={() => setFilter('all')}>すべて</button><button className={filter === 'marked' ? 'selected' : ''} onClick={() => setFilter('marked')}><Star size={18} fill={filter === 'marked' ? 'currentColor' : 'none'} /> マーク</button></nav></div><div className="category-group"><span className="filter-group-label">カテゴリ</span><nav className="category-tabs" aria-label="カテゴリの切り替え">{categories.map((category) => <button key={category.number} className={categoryFilter === category.number ? 'selected' : ''} onClick={() => setCategoryFilter((current) => current === category.number ? null : category.number)} aria-pressed={categoryFilter === category.number}><b>{category.number}</b>{category.name}</button>)}</nav><small>同じカテゴリをもう一度押すと、すべて表示します。</small></div></> : <p className="guide-hint"><Camera size={20} />1つの操作項目に、画像＋説明の手順を最大10件まで保存できます。</p>}
    <section className={`memo-list ${section === 'pc-linux' ? 'guide-list' : ''}`} aria-live="polite">
      {loading ? <p className="status">読み込み中…</p> : displayed.length === 0 ? <p className="status">まだ{section === 'daily' ? 'メモ' : '操作項目'}がありません。<br />上のボタンから追加できます。</p> : displayed.map((memo) => section === 'daily' ? <article className="memo-row" key={memo.id}><button className={`star-button ${memo.marked ? 'marked' : ''}`} onClick={() => void toggleMark(memo)} aria-label={memo.marked ? 'マークを外す' : 'マークする'}><Star fill={memo.marked ? 'currentColor' : 'none'} /></button><span className="memo-number">{memo.displayNumber}.</span><button className="memo-content" onClick={() => openEdit(memo)}><strong>{memo.title}</strong><span className="memo-category">{memo.categoryNumber} {categoryName(categories, memo.categoryNumber)}</span>{memo.meaning && <span>{memo.meaning}</span>}</button><button className="icon-button edit" onClick={() => openEdit(memo)} aria-label="編集"><Edit3 size={22} /></button></article> : <article className="guide-card" key={memo.id}><button className="guide-card-main" onClick={() => setViewingGuide(memo)}>{memo.steps[0]?.imageDataUrl ? <img src={memo.steps[0].imageDataUrl} alt="" /> : <span className="guide-placeholder"><ImagePlus /></span>}<span className="guide-card-copy"><small>操作項目 {memo.displayNumber}</small><strong>{memo.title}</strong><span>{memo.steps.length}手順</span></span></button><button className="icon-button guide-edit" onClick={() => openEdit(memo)} aria-label={`${memo.title}を編集`}><Edit3 size={22} /></button></article>)}
    </section>

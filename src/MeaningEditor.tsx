...
export function MeaningEditor({ title, value, onChange, onApply, onDictate }: Props) {
  ...
  return <section className="meaning-section" aria-label="意味・説明の入力">
    <div className="title-tools">
      <button type="button" className="dictation" onClick={() => { speech.stop(); onDictate() }}><Mic size={20} aria-hidden="true" />話して入力</button>
    ...
    <div className="meaning-heading">
      <label htmlFor="memo-meaning">意味・説明</label>
      <button type="button" className="meaning-button" onClick={() => void lookup()} disabled={loading} aria-describedby="meaning-help" aria-controls="memo-meaning">
        <Search size={20} aria-hidden="true" />{loading ? '調べています…' : '説明を調べる'}
      </button>
    </div>
    <p id="meaning-help" className="meaning-help">Wikipediaから説明を入力できます。</p>
    <textarea id="memo-meaning" value={value} onChange={(event) => onChange(event.target.value)} placeholder="説明を直接書くこともできます" rows={5} maxLength={2000} />
